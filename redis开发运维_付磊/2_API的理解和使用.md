...
```txt
127.0.0.1:6380> brpop list:test 3
(nil)
(3.05s)
```
如果在此期间增加了元素，客户端立即返回
...
列表类型的内部编码有两种
ziplist 压缩列表：当元素个数小于list-max-ziplist-entries配置(默认512个)，同时列表中每个元素的值都小于list-max-ziplist-value配置时(默认64字节)，redis会使用ziplist作为列表内部实现来减少内存的使用。

linkedlist 链表：无法满足ziplist的条件，redis就使用linkedlist作为列表的内部实现
...
4) "4"
5) "5"
127.0.0.1:6380> object encoding list
"quicklist"
```
redis 3.2版本提供了quicklist内部编码，简单的说它是一个ziplist为节点的linklist,它结合了ziplist和linkedlist两者yoush优势。
...
redis的lpush+brpop命令组合即可实现阻塞队列，生产者客户端使用lrpush从列表左侧插入元素，多个消费者客户端使用brpop命令阻塞式的抢到列表尾部的元素，多个客户端保证了消费的负载均衡和高可用性。

2. 文章列表

每个用户有属于自己的文章列表，现在需要分页展示文章列表，此时可以考虑使用列表，因为列表不但是有序的，而且同时支持按照索引范围获取元素。

- 每篇文章使用hash结构存储，例如每篇文章有3个属性(title,timestamp,content)
...
集合set类型也是用来保存多个字符串的元素，但是和列表类型不一样的是，集合中不允许重复元素，并且集合元素是无序的，不能通过下标获取元素，一个集合最多可以存储2^32 - 1个元素，redis除了支持集合内的增删改查，同时支持多个集合取交集、并集、差集
...
添加元素 sadd key element [element ...],返回结果为成功添加元素的个数,可以不断添加新元素自动去重

```txt
127.0.0.1:6380> sadd myset a b c d e
(integer) 5
127.0.0.1:6380> sadd myset f g
(integer) 2
```
删除元素 srem key element [element ] 返回结果为成功删除元素的个数

```txt
127.0.0.1:6380> srem myset b c d e
(integer) 4
```
计算set元素的个数scard key
...
```txt
127.0.0.1:6380> sismember myset a
(integer) 1
```
随机从集合中返回指定个元素srandmember key [count]
...
3) "f"
```
[count]是可选参数，如果不选则默认为1

从集合中随机弹出元素spop key [count],(弹出后集合中就少了弹出的那个元素)，spop是随机弹出元素，从redis 3.2开始，spop 支持[count]属性
...
2) "a"
```
srandmember 和spop都是随机从集合中弹出元素，不同的是spop命令执行后，元素会从集合中删除，而srandmember不会删除元素

获取所有元素 smembers key(无序的)
...
现在有两个集合user:1:follow 和user:2:follow

127.0.0.1:6383> sadd user:1:follow it music his sports
(integer) 4
127.0.0.1:6383> sadd user:2:follow it news ent sports
(integer) 4

交集sinter key [key ...]
127.0.0.1:6383> sinter user:1:follow user:2:follow
1) "sports"
2) "it"

求并集sunion key [key ...]
127.0.0.1:6383> SUNION user:1:follow sunion user:2:follow
1) "ent"
2) "it"
3) "news"
4) "his"
...
127.0.0.1:6383> sdiff user:1:follow user:2:follow
1) "his"
2) "music"
...
```
集合间的运算在元素比较多的时候比较费时，redis将三个命令(原命令+store)将结果保存在destination key中，destination key也是一个新的集合

```txt
127.0.0.1:6383> sinterstore user:1_2:follow_inter user:1:follow user:2:follow
(integer) 2
127.0.0.1:6383> sunionstore user:1_2:follow_union user:1:follow user:2:follow
(integer) 6
127.0.0.1:6383> sdiffstore user:1_2:follow_diff user:1:follow user:2:follow
(integer) 2
127.0.0.1:6383> smembers user:1_2:follow_inter
1) "it"
2) "sports"
127.0.0.1:6383> smembers user:1_2:follow_union
1) "ent"
2) "it"
3) "news"
4) "his"
5) "sports"
6) "music"
127.0.0.1:6383> smembers user:1_2:follow_diff
1) "his"
2) "music"
```
...
集合常用命令时间复杂度O
- 命令                      时间复杂度O
- sadd key [key ...]       O(k) ,k是元素个数
- srem key [key ...]       O(k) ,k是元素个数
- scard key                O(1)
- sismember key element    O(1)
- srandmember key [count]  O(count)
- spop key                 O(1)
- smembers key             O(n),n是元素总数
- sinter key [key ...]或者sinterstore   O(m*k) k是多个集合元素最少的个数，m是键个数
- sunion key [key ...]或者sunionstore   O(k),k是多个集合元素个数和
...
intset(整数集合)：当元素都是整数且个数少于set-max-intset-entries配置(默认512个)时，redis会选择intset作为集合内部实现

hashtable(哈希表)，当集合类型无法满足intset时，使用hashtable作为集合内部实现

```txt
127.0.0.1:6383> object encoding user:1:follow
"hashtable"
```
...
集合比较典型的使用场景是标签tag，一个用户可能对体育、娱乐感兴趣，另一用户对历史、新闻、体育比较感兴趣，可以快速找到有共同爱好的人，这对于用户体验以及增强用户黏度比较重要。电子商务网站广告推荐

给用户添加标签
```txt
127.0.0.1:6383> sadd user:1:tags tag1 tag2 tag5
(integer) 3
127.0.0.1:6383> sadd user:2:tags tag2 tag3 tag5
(integer) 3
127.0.0.1:6383> sadd tag1:users user:1: user:3
(integer) 2
```
删除用户下的标签
...
2.6 有序集合zset(是集合元素就不重复，有序就按指定顺序排列)
===
...
添加成员zadd key score member [score member ...],返回结果是添加成员个数

```txt
127.0.0.1:6383> zadd user:ranking 1 kris 91 mike 200 frank 100 tim 90 bill
(integer) 5
```
- redis 3.2增加了nx、xx、ch、incr四个选项
- nx: member必须不存在，才可以设置成功，用于添加
- xx: member 必须存在，才可以设置成功，用于更新
- ch: 返回此次操作后，有序集合元素和分数发生变化的个数
- incr: 对score做增加，相当于zincrby

有序集合比集合提供了排序字段，但是也产生了代价，zadd的时间复杂度为O(log(n)),sadd的时间复杂度为O(1)

计算成员个数zcard key

计算某个成员的分数zscore key member

计算成员的排名zrank key member   /zrevrank key member
...
```txt
127.0.0.1:6383> zadd user:ranking 251 tom
(integer) 1
127.0.0.1:6383> zadd user:ranking 1 kris 91 mike 200 frank 100 tim 90 bill
(integer) 5
127.0.0.1:6383> zincrby user:ranking 9 tom
"260"
```
